...
//Kafka Producer - starting to produce a message
...
log.info("Kafka Producer AdminSystem  is:" + eventBody.get("AdminSystem"))
log.info("Kafka producer GUID  is:" + eventBody.get("GUID"))
String GUIDProducer = eventBody.get("GUID")
...
if(rs.next()){
    ...
    String adminSystemDB = jsonPayload.getString("adminSystem")
    String primaryHoldingKeyDB = jsonPayload.getString("primaryHoldingKey")

    if (guidDB.equals(GUIDProducer)) {

        log.info("Kafka Producer Primary_Holding_ID is:" + eventBody.get("Primary_Holding_ID1"))
        log.info("DB Primary_Holding_ID is:" + primaryHoldingKeyDB)

        log.info("Kafka Producer AdminSystem  is:" + eventBody.get("AdminSystem"))
        log.info("DB AdminSystem  is:" + adminSystemDB)

        conn.close()

        log.info("DB Validations for guid: " + guidDB + "admin System DB: " + adminSystemDB +  "Primary_Holding_ID: "+ primaryHoldingKeyDB + "passed successfully" )
    }
}