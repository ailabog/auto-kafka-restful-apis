...
String dateCreatedNow = inputFormat.format(now)
...
eventBody.put("DateCreated", dateCreatedNow)
...
for(ConsumerRecords<String, GenericRecord> record : records) {
    ...
    if(eventCorrelationId.equals(eventCorrelationIdConsumer)) {
        ...
        softly.assertThat(avroRecordConsum.get("eventBody.DateCreated").toString()).isEqualTo(dateCreatedNow)
