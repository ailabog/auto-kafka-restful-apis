...
eventBody.put("Primary_Holding_ID1", new GenericData.Array<>(avroSchemaEbill.getField("eventBody").schema().getField("Primary_Holding_ID1").schema(), Arrays.asList("00000000000025718037")))
eventBody.put("AdminSystem", new GenericData.Array<>(avroSchemaEbill.getField("eventBody").schema().getField("AdminSystem").schema(), Arrays.asList("VNTG1")))
...
if(roleType.equals(expectedRole) && (memberGUIDAgreementCustomer).equals(memberGUIDCustomerId)) {
    ...
    log.info("Agreement key from Agreement customer MS"   + agreementKeyMS)
}
